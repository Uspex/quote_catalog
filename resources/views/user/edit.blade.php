...
@section('page_title', __('Редактирование пользователя'))
@section('page_excerpt', __('Редактирование пользователя'))
                    ...
                    <div class="nk-block-head nk-block-head-sm">
                        <div class="nk-block-between g-3">
                            <div class="nk-block-head-content">
                                <h3 class="nk-block-title page-title">{{ __('Редактирование пользователя') }} </h3>
                            </div>
                            <div class="nk-block-head-content">
                                <a href="{{ route('user.index') }}" class="btn btn-outline-light bg-white d-none d-sm-inline-flex"><em class="icon ni ni-arrow-left"></em><span>{{ __('Назад') }}</span></a>
                            </div>
                        </div>
                    </div><!-- .nk-block-head -->
                    <div class="nk-block">
                                                                    ...
                                                                    <div class="form-control-wrap">
                                                                        <select name="role" class="form-select form-control form-control-lg" required>
                                                                            ...
                                                                            @foreach($roles as $role)
                                                                                <option value="{{ $role->name }}" @if($item->hasRole($role->name)) selected @endif>{{ $role->display_name }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </div>
                                                                </div>
